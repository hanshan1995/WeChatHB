...
public class AccessibilityHelper {
    ...
    public static final String TAG="spli_AccessibilityHelp";
    public static void testEventInfo(AccessibilityService service, AccessibilityEvent event){
        try {
            ...
            Log.i(TAG, "testEventInfo: packageName="+packageName);
            Log.i(TAG, "testEventInfo: className="+className);
            //获取当前界面父布局的控件
            AccessibilityNodeInfo accessibilityNodeInfo=service.getRootInActiveWindow();
            if (accessibilityNodeInfo!=null){
                int childCount=accessibilityNodeInfo.getChildCount();
                Log.i(TAG, "testEventInfo: 父布局下的子节点个数="+childCount);
                CharSequence className1 = accessibilityNodeInfo.getClassName();
                Log.i(TAG, "父布局的类名  " + className1);

                CharSequence text = accessibilityNodeInfo.getText();
                Log.i(TAG, "父布局的text  " + text);
                ...
                if (childCount > 0) {
                    for (int i = 0; i < childCount; i++) {
                        ...
                        Log.i(TAG, "父布局的子节点类名  " + childName);
                        boolean clickable = child.isClickable();
                        Log.i(TAG, "父布局的子节点  " + (clickable ? "可点击" : "不可点击"));
            ...
            if (accessibilityNodeInfo != null) {
                ...
                if (nodeInfoList != null && nodeInfoList.size() > 0) {
                    for (int i = 0; i < nodeInfoList.size(); i++) {
                        ...
                        Log.i(TAG, "通过内容找到的节点类名  " + nodeInfo.getClassName());
                    }
                    ...
                    if (lastNode != null) {
                        ...
                        if (parent != null) {
                            ...
                            Log.i(TAG, "包含内容  " + text + " 的节点父节点下的子节点的个数 " + childCount);
                            if (childCount > 0) {
                                ...
                                for (int i = 0; i < childCount; i++) {
                                    ...
                                    if (child != null) {
                                        ...
                                        Log.i(TAG, "包含内容  " + text + " 的节点父节点下的子节点的类名 " + className);

                                        if ("android.widget.TextView".equals(className)) {
                                            String contentText = child.getText().toString();
                                            Log.i(TAG, "包含内容  " + text + " 的节点父节点下的子节点的text为 " + contentText);
    ...
    public static boolean clickHongbao(AccessibilityNodeInfo parent) {
        if (parent != null) {
            ...
            Log.i(TAG, "parent is click " + result);
    ...
    public static void openPackage(AccessibilityService service) {
        try {
            ...
            if (accessibilityNodeInfo != null) {
                int childCount = accessibilityNodeInfo.getChildCount();
                Log.i(TAG, "父布局下的子节点个数 " + childCount);
                ...
                if (childCount > 0) {
                    for (int i = 0; i < childCount; i++) {
                        ...
                        if ("android.widget.TextView".equals(childName)) {
                            ...
                            Log.i(TAG, "text view content " + textContent);
                        }
        ...
        boolean b = service.dispatchGesture(gesture,
                new AccessibilityService.GestureResultCallback() {
                    ...
                    public void onCompleted(GestureDescription gestureDescription) {
                        ...
                        Log.i(TAG, "dispatch gesture success ");
                    }
                    ...
                    public void onCancelled(GestureDescription gestureDescription) {
                        ...
                        Log.i(TAG, "dispatch gesture failure ");

                    }
                }, null);

        Log.i(TAG, "dispatch return is " + b);
    }
    ...
    public static void dispatchGestureClick(AccessibilityService service) {
        ...
        Log.d(TAG, "device screen dpi " + dpi);

        int heightPixels = metrics.heightPixels;
        int widthPixels = metrics.widthPixels;

        // width 1080   height 2250
        Log.i(TAG, "screen height pixels " + heightPixels);
        Log.i(TAG, "screen width pixels " + widthPixels);
